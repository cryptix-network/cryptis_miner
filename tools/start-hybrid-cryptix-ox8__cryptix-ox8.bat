...
REM CPU+GPU hybrid: CPU cryptix+ox8 and GPU cryptix+ox8

set POOL_URL=stratum+tcp://ox8-cpu-pool.example.com:3333
set WALLET=cryptix:YOUR_CRYPTIX_WALLET
...
set EXTRA_ARGS=%EXTRA_ARGS% --gpu-coin cryptix --gpu-hash ox8

if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin cryptix --hash ox8 %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin cryptix --hash ox8 --threads %THREADS% %EXTRA_ARGS%
)
