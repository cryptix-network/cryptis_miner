...
REM CPU+GPU hybrid: CPU cryptix+ox8 and GPU unknown+autolykosv2
...
set CPU_WALLET=cryptix:YOUR_CRYPTIX_WALLET
set CPU_USER=
set CPU_PASSWORD=x
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin cryptix --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash autolykosv2 %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin cryptix --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash autolykosv2 --threads %THREADS% %EXTRA_ARGS%
)
