...
REM CPU+GPU hybrid: CPU unknown+ox8 and GPU unknown+autolykosv2
...
set CPU_WALLET=YOUR_UNKNOWN_OR_GENERIC_WALLET
set CPU_USER=
set CPU_PASSWORD=x

set GPU_POOL_URL=stratum+tcp://ergo-gpu-pool.example.com:4444
set GPU_FAILOVER_URLS=
set GPU_WALLET=YOUR_POOL_LOGIN_OR_WALLET
set GPU_USER=
set GPU_PASSWORD=x
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash autolykosv2 %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash autolykosv2 --threads %THREADS% %EXTRA_ARGS%
)
