...
REM CPU+GPU hybrid: CPU unknown+randomx and GPU unknown+ox8
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash randomx --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash ox8 --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR% %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash randomx --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash ox8 --threads %THREADS% --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR% %EXTRA_ARGS%
)
