...
REM CPU-only: ergo + autolykosv2 (reference verifier path)

set POOL_URL=stratum+tcp://ergo-cpu-pool.example.com:4444
set WALLET=YOUR_ERGO_WALLET
set WORKER_NAME=rig01
set POOL_PASSWORD=x
set THREADS=

if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin ergo --hash autolykosv2 --no-gpu
) else (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin ergo --hash autolykosv2 --no-gpu --threads %THREADS%
)
