...
REM GPU-only: OpenCL compatibility profile for hoohash (legacy filename)
...
"%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin unknown --hash hoohash --no-cpu %EXTRA_ARGS%
