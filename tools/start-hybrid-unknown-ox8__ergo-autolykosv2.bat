...
REM CPU+GPU hybrid: CPU unknown+ox8 and GPU ergo+autolykosv2
...
set GPU_WALLET=YOUR_ERGO_WALLET
set GPU_USER=
set GPU_PASSWORD=x
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin ergo --gpu-hash autolykosv2 %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash ox8 --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin ergo --gpu-hash autolykosv2 --threads %THREADS% %EXTRA_ARGS%
)
