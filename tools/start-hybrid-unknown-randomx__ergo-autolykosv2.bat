...
REM CPU+GPU hybrid: CPU unknown+randomx and GPU ergo+autolykosv2
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash randomx --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin ergo --gpu-hash autolykosv2 --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR% %EXTRA_ARGS%
) else (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin unknown --hash randomx --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin ergo --gpu-hash autolykosv2 --threads %THREADS% --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR% %EXTRA_ARGS%
)
