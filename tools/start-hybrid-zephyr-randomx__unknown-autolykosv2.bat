...
REM CPU+GPU hybrid: CPU zephyr+randomx and GPU unknown+autolykosv2
...
set CPU_WALLET=YOUR_ZEPHYR_WALLET
set CPU_USER=
set CPU_PASSWORD=x
...
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %CPU_POOL_URL% --wallet %CPU_WALLET% --worker %WORKER_NAME% --password %CPU_PASSWORD% --coin zephyr --hash randomx --cpu-pool %CPU_POOL_URL% --cpu-wallet %CPU_WALLET% --cpu-password %CPU_PASSWORD% --gpu-pool %GPU_POOL_URL% --gpu-wallet %GPU_WALLET% --gpu-password %GPU_PASSWORD% --gpu-coin unknown --gpu-hash autolykosv2 --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR% %EXTRA_ARGS%
) else (
