...
set RANDOMX_MSR=off
if "%THREADS%"=="" (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin unknown --hash randomx --no-gpu --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR%
) else (
    "%MINER_EXE%" --pool %POOL_URL% --wallet %WALLET% --worker %WORKER_NAME% --password %POOL_PASSWORD% --coin unknown --hash randomx --no-gpu --threads %THREADS% --randomx-hugepages %RANDOMX_HUGEPAGES% --randomx-msr %RANDOMX_MSR%
)
